...
This app allows a user to browse through an OPT volume and (eventually) add annotations about probe location.
...
class App(QWidget):
    ...
    def initUI(self):
        ...
        template_dir = os.path.join(base_dir, 'template_brain')
        self.current_directory = base_dir
        template_path = os.path.join(template_dir, 'template_fluor.pvl.nc.001')
        self.template_annotations = np.load(os.path.join(template_dir, 'landmark_annotations.npy'))

        # np.zeros((NUM_LANDMARK_SLICES * NUM_LANDMARKS_PER_SLICE,3 )) - 1
