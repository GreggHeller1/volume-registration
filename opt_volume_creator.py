...
# -*- coding: utf-8 -*-
"""
Created on Tue Aug 14 11:28:32 2018

@author: joshs

This script contains code for converting a series of post-reconstruction OPT images to a volume.
...
def open_image(filename, rotation, x_offset, y_offset, imwidth):
    ...
    imarray = resize(imarray, (1024, 1024), mode='reflect') * 255
...
def resize_volume(volume):
    ...
    for horzslice in range(1024):
        ...
        imslice = resize(imslice, (1024, 1024), mode='reflect')
        imslice = imslice*255
...
for mouse in info['Mouse'].values[53:]:
    ...
    #try:
        ...
        for type_index, image_type in enumerate(image_types):
            ...
            remote_directory = create_samba_directory(row.Location[:3], os.path.join(row.Location[4:], str(mouse)))
            ...
            print("   Saving volume...")
            save_volume(volumeT, 'mouse' + str(mouse),'/mnt/md0/data/opt/production', image_type)
...
for mouse in info['Mouse'].values[1:]:
...
for mouse in info['Mouse'].values[0:]:
